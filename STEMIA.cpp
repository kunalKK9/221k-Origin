...
void autonomous() {
    ...
    indexer.set_value(true); //put indexer up at beginning
    chassis.moveToPoint(0, 32, 1500,{.maxSpeed = 80}); //go ahead of the match loader
    chassis.turnToHeading(90,500); //turn to match loader
    scraper.set_value(true);
    chassis.moveToPose(25.5,23.39,90,1000); //into match loader
    intake.move(127);
    Indexermotor.move(127); 
    pros::delay(1250); //intake until 3 balls
    intake.move(0);
    Indexermotor.move(0); //stop intaking
    chassis.moveToPoint(-1, 23, 1500,{.forwards = false}); //move back most of the way
    scraper.set_value(false);
    pros::delay(500);
    Indexermotor.move(-127); //put the ball a little away from the nope
    intake.move(-127);
    pros::delay(120);
    Indexermotor.move(0); //stop moving the intakes
    intake.move(0);

    nope.set_value(true); //put the nope up

    chassis.moveToPoint(-28.5, 23,1500,{.forwards = false, .maxSpeed = 60});
