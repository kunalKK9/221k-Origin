...
void scraperNopeControl(bool mode, bool scraperToggle) {
    if (!scraperToggle) {
        ...
        if (!mode) {
            nope.set_value(false);  // Hoard mode -> nope up
        } else {
            nope.set_value(true);   // eject mode -> nope down
        }
    }
}
...
void disabled() {
    ...
    while (true) {
        pros::delay(100); // small delay to prevent shitting itself 
    }
}
...
void opcontrol() {
    ...
    scraperNopeControl(mode, scrapertoggle);

    // toggle scraper when L2 is pressed
    if (controller.get_digital_new_press(pros::E_CONTROLLER_DIGITAL_L2)) {
        scrapertoggle = !scrapertoggle; // flip scraper state
    }
    ...
    controller.print(0, 0, "Mode: %s", mode ? "EJECT" : "HOARD");
